...
func TestAddEvent(t *testing.T) {
	s := New()
	event := Event{ID: 1, Title: "Test Event", Time: timeNow}
...
func TestUpdateEvent(t *testing.T) {
	...
	nonExistentEvent := Event{ID: 2, Title: "Non-existent Event", Time: timeNow}
	...
	err = s.AddEvent(event)
	require.NoError(t, err)
	updatedEvent := Event{ID: 1, Title: "Updated Event", Time: timeNow}
...
func TestListEvents(t *testing.T) {
	s := New()
	event1 := Event{ID: 1, Title: "Test Event 1", Time: timeNow}
	event2 := Event{ID: 2, Title: "Test Event 2", Time: timeNow}
...
func TestConcurrentAccess(t *testing.T) {
	...
	event1 := Event{ID: 1, Title: "Test Event 1", Time: t1}
	event2 := Event{ID: 2, Title: "Test Event 2", Time: t2}
	...
	go func() {
		updatedEvent := Event{ID: 1, Title: "Updated Event 1", Time: t1}
		err := s.UpdateEvent(updatedEvent)
		require.NoError(t, err)
		done <- true
	}()
